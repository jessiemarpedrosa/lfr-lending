		...
		if(mysqli_num_rows($result) > 0){
			echo '<div class="filterResults loanDetailsList"><table class="table table-bordered table-striped">';
                ...
                <div id="promi_note" class="promi_content" data-loan-id="<?= $row['id'] ?>">
                    ...
                    $loan_no = $row['loan_no'];
                    $sql_get_payments = "SELECT SUM(amt_received) as total_paid FROM lfr_transactions WHERE loan_no = '$loan_no'";
                    ...
                    // Calculate interest using actual loan interest rate from DB
                    $interest_amount = $row['totalloanamt'] * ($row['loaninterestrate'] / 100);
                    $totalWithInterest = $row['totalloanamt'] + $interest_amount;

                    // Outstanding balance without penalty (loan + interest - payments)
                    $temp_outstanding_balance = $totalWithInterest - $total_paid;

                    // Calculate penalty (2% per month × rounded months, based on temp outstanding balance)
                    $penalty = 0;
                    $months_elapsed = 0;
                    ...
                    if ($loan_end_date && $current_date > $loan_end_date) {
                        ...
                        $months_elapsed = round($days_overdue / 30);
                        $penalty = round(($temp_outstanding_balance * 0.02) * $months_elapsed, 2);
                    }

                    $total_amount_due = round($temp_outstanding_balance + $penalty, 2);
                    ?>
